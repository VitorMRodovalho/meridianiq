...
- **PR #64** — Convention normalization (Cycle 3 close-arc hygiene): consolidated all `@limiter.limit`-decorated body-bearing endpoints to a single convention (`request: Request, body: <Pydantic | dict>`). Migrated 9 endpoints from prior conventions:
  ...
  Pure hygiene; no functional change (all 3 prior conventions satisfied slowapi Rule 4 and FastAPI body auto-binding). ADR-0019 Amendment 1 §"Negative / accepted costs" updated to reflect single-convention codebase.
