...
- **PR #60** — `#57` (P3, security): rate-limit decorator parameter rename closing the ADR-0019 Amendment 1 deferral. 6 endpoints (`analysis.contract_check`, `forensics.create_timeline`, `schedule_ops.build_schedule_endpoint`, `whatif.run_what_if`, `whatif.run_pareto_analysis`, `whatif.run_resource_leveling`) renamed `request: <Pydantic>` → `body: <Pydantic>` + added `request: Request` first parameter + decorated with the appropriate bucket (2 × `RATE_LIMIT_MODERATE` + 4 × `RATE_LIMIT_EXPENSIVE`). Empirical: write-with-limiter count 30 → 36 (issue body's projected "44" miscounted; PR #56's 8 additions were already in the 30 baseline); APPROVED_EXCEPTIONS 8 → 2 (only the 2 user-self-action `require_auth`-gated endpoints remain). All 5 rate-limit policy tests still pass. Adjacent observation surfaced for follow-up: `forensics.py` MIP endpoints use a different convention (`_http_request: Request` sibling) — both work; #57 stayed with the issue spec to match `optimize_schedule_endpoint` precedent. Closes #57.
