# DAX Measures — Xer_Reader_PQ_sqlite
...
## Metrics.Missing Successor
```dax
if(CALCULATE(sum(TASK[succesor]))=0,BLANK(),CALCULATE(sum(TASK[succesor])))
```
...
## Metrics.Forecast_Hours_Period
```dax
CALCULATE(sum(Forecast[Forecast_Hours]))
```

## Metrics.Remaining_Hours_Cumulative
```dax
if(COUNTROWS(VALUES(Mstdate[Date])) = 1,
	SUMX(DATESBETWEEN(Mstdate[Date], FIRSTDATE(TASK[Data_Date]), VALUES(Mstdate[Date]))
			, Metrics[Forecast_Hours_Period])
, BLANK())
```

## Metrics.Forecast_Hours_Cumulative
```dax
CALCULATE([Forecast_Hours_Period],filter(all(Mstdate),Mstdate[Date]<=max(Mstdate[Date])))
```
