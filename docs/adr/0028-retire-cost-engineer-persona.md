...
3. **Carved out of #31 before closure, because it is a defect in shipped code rather than planned scope:** `save_cost_upload` keys cost snapshots on `project_id` only — no `input_hash`, no link to a schedule revision. A cost figure quoted in a forensic narrative therefore cannot be tied to the revision that produced it. That is a chain-of-custody gap on the **Consultant/Claims SME**'s surface, not the Cost Engineer's, and it does not die with this persona. It was re-filed on 2026-09-04 as **#243** (`bug` / P2 / `area:schema` / `parked`), ahead of any closure and independent of the gate outcome, so that a real integrity defect in shipped code cannot die inside a scope closure.
...
* **Cycle 7 inherits the Cycle 6.5 obligations.** Enumerated here rather than referenced, because every other place they are written is conditional ("IF the gate is met cosmetically…") and the one artifact that tracked them as a live item is closed by this decision:
  1. **Cycle 7 pivots to discovery-only** (ADR-0025 §Decision W2 branch: *"IF NOT MET → BLOCK + amend this ADR → Cycle 6.5 pivot to discovery-only"*; operator decision 5 at PR #135). No feature waves at Cycle 7 entry. *[OVERRIDDEN: see [ADR-0027 §"Owner override (2026-09-25)"](0027-park-maintenance-only.md#owner-override-2026-09-25-park-lifted-without-demand-evidence). Cycle 7 is not discovery-only; the discovery obligation continues in parallel and is logged on #134.]*
  ...
  3. **The Cycle 6 release tag** (v4.4.0 or v4.3.1) stays frozen per ADR-0027, unchanged by this decision. *[OVERRIDDEN: see [ADR-0027 §"Owner override (2026-09-25)"](0027-park-maintenance-only.md#owner-override-2026-09-25-park-lifted-without-demand-evidence) in part. The park no longer freezes the tag; the canonical-identity order recorded there does.]*
* **#134 is NOT closed.** The first draft closed it, which would have removed the only live artifact tracking the inherited obligation at the exact moment the obligation became real. Instead the outcome commit **retitles** it to carry the qualifier in the title — where GitHub renders it on every cross-reference, hovercard and backlink — and it is re-labelled as the Cycle 7 entry blocker:
  ...
  *[OVERRIDDEN: see [ADR-0027 §"Owner override (2026-09-25)"](0027-park-maintenance-only.md#owner-override-2026-09-25-park-lifted-without-demand-evidence). If Pathway B executes, the retitle reads *"Cycle 6 W2 HARD GATE — met via Pathway B, COSMETICALLY (discovery-only pivot set aside by owner override 2026-09-25; outreach obligation continues)"*.]*
...
* **The `parked` state of ADR-0027 is unaffected.** Executing this ADR unparks nothing; the repository remains maintenance-only, and Cycle 7 entry remains frozen and now additionally encumbered. *[OVERRIDDEN: see [ADR-0027 §"Owner override (2026-09-25)"](0027-park-maintenance-only.md#owner-override-2026-09-25-park-lifted-without-demand-evidence). The park was lifted by owner decision on 2026-09-25, before this outcome commit; executing this ADR neither re-parks nor unparks.]*
