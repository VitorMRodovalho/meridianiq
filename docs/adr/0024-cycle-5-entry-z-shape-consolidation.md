...
6. **Maintainer financial runway is binary blocker for any active-discovery component**. Per Round 1 strategist + Round 2 IV. Without runway disclosure (even bracketed: < 6mo / 6-12mo / 12-24mo / > 24mo), Z-IV components are undecidable. **Mitigation upgraded per DA exit-council P0 #3**: runway disclosure is **PRE-RATIFICATION GATE** — this PR cannot merge until §"Honest disclosures" #2 + #3 are filled with maintainer-provided bracket + contact-path inventory. Earlier draft had runway as post-ratification W0 deliverable which silently weakened Round 2 IV's explicit demand: *"council should refuse to ratify any path until runway is on the table."* IV's bright line is honored here.
...
2. **Maintainer financial runway** (PRE-RATIFICATION GATE per DA exit-council P0 #3): **{PENDING MAINTAINER DISCLOSURE — this PR cannot merge until this placeholder is filled with one of: <6mo / 6-12mo / 12-24mo / >24mo}**. The disclosure is bracket-only; no exact figure required. Round 2 IV demanded pre-ratification disclosure ("council should refuse to ratify any path until runway is on the table"); earlier draft demoted to W0 post-ratification deliverable, weakening IV's bright line; corrected here. If <6mo, all Z-IV W5 components drop automatically and cycle is pure Z-DA.

3. **AACE TCM / PMI-CP membership status + contact-path inventory** (PRE-RATIFICATION GATE per DA exit-council P0 #3): **{PENDING MAINTAINER DISCLOSURE — this PR cannot merge until this placeholder is filled with either (a) prior contacts + membership status documented, OR (b) explicit "0 contacts, no membership, willing/unwilling to apply"}**. Z-IV W5 outreach component gates on this. If "0 contacts, no membership, unwilling to apply", the AACE/PMI outreach sub-component of Z-IV W5 drops automatically; remaining sub-components (paid-product conversation, Issue #13 refresh) still executable.
