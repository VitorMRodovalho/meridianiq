...
- Dockerfile: Python 3.13-slim — pinned because `supabase` pulls `storage3` → `pyiceberg` (transitive), and `pyiceberg` has no Python 3.14 wheel for slim Linux as of 2026-04-26 (build-from-source fails on missing system headers). Bump to 3.14 only after pyiceberg ships 3.14 wheels OR storage3 drops the dep. Revert reason: deploy break in commit 28da1fd → reverted in next commit.
