...
- **API**: FastAPI with 113 endpoints under `/api/v1/` across 20 routers, rate-limited critical endpoints
...
- Version: v3.8.0 (released 2026-04-18) — "Forensic MIP Expansion + Frontend Hardening" (45 engines + 1 export module, 22 MCP tools, 1084 tests, 11 chart components, 113 endpoints across 20 routers, 54 pages, 15 PDF report types). v3.8.0 features: BI templates (Power BI/Tableau/LookML) + AIA G702 Certificate PDF, five new AACE forensic MIPs (3.1/3.2/3.5/3.6/3.7 — 7 of 9 now covered), per-WBS Gantt print, submission-deliverables user guide, stats-consistency CI validator, schedule-view cache invalidation, audit IP/UA capture, CBS compare picker filter, two type-safety sweeps (all `any` removed from routes), full i18n wiring across layout + 23 pages (login/projects/programs/settings/org/pareto/contract/forensic/recovery/ips/risk-register/builder/scorecard/narrative/cost/trends/demo/visualization/org-detail/milestones/compare/docs plus layout chrome).
