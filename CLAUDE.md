...
- **45 analysis engines** in `src/analytics/` + 1 export module in `src/export/` — each standalone, no cross-dependencies
- **API**: FastAPI with 116 endpoints under `/api/v1/` across 22 routers, rate-limited critical endpoints
...
- **Database**: Supabase PostgreSQL with RLS, 23 migrations in `supabase/migrations/`
...
- Version: v3.9.0 (released 2026-04-18) — "Real-Time + Extensibility" (45 engines + 1 export module, 22 MCP tools, 1148 tests, 11 chart components, 115 endpoints across 21 routers, 54 pages, 15 PDF report types). v3.9.0 features: WebSocket progress streaming for Monte Carlo (`/api/v1/ws/progress/{job_id}`), third-party plugin architecture with entry-point discovery + HTTP surface (`GET /api/v1/plugins`, `POST /api/v1/plugins/{name}/run/{project_id}`), MCP server HTTP/SSE transports (`--transport stdio|http|sse`), activity grouping by status/critical/task_type/calendar/float_bucket, structured P6 `clndr_data` parser (CalendarSchedule + CalendarException distinguishing holidays from working exceptions), mobile responsiveness pass on hot-spot pages, in-memory KPI cache for `/programs/rollup` + `/bi/projects` (120s TTL), slowapi Limiter consolidation. P2 backlog 100% closed; first 5 P3 items closed (MCP HTTP, mobile, calendar exceptions, plugin arch, activity grouping, WS progress).
