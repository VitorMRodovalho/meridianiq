...
- **API**: FastAPI with 112 endpoints under `/api/v1/` across 20 routers, rate-limited critical endpoints
...
- Version: v3.7.0 (released 2026-04-17) — "Forensic & Executive Reporting" (41 engines + 1 export module, 22 MCP tools, 943 tests, 11 chart components, 107 endpoints across 20 routers, 53 pages, 14 PDF report types). v3.7.0 features: CBS snapshot compare (`/cost/compare`), Program rollup endpoint + 8-KPI dashboard, Executive Summary PDF (with attribution / CBS variance / program context), Risk register persistence + simulation↔register linkage, SupabaseStore CBS rehydration, BI connector (`/api/v1/bi/*` paginated surfaces), SCL Protocol + AACE RP 29R-03 §5.3 submission PDFs, AIA G703 Continuation Sheet Excel export, Gantt SVG/PNG export + print CSS.
