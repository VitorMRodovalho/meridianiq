...
<div class="p-8 max-w-6xl mx-auto">
	<div class="mb-8">
		<h1 class="text-2xl font-bold text-gray-900">{$t('page.evm')}</h1>
	...
	{#if latest}
		<div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
			<div class="bg-white rounded-lg border border-gray-200 p-4 text-center">
				...
				<p class="text-xs text-gray-500 uppercase mt-1">SPI</p>
			...
			<div class="bg-white rounded-lg border border-gray-200 p-4 text-center">
				...
				<p class="text-xs text-gray-500 uppercase mt-1">CPI</p>
				<div class="mt-2 h-1.5 rounded-full bg-gray-100 overflow-hidden">
			...
			<div class="bg-white rounded-lg border border-gray-200 p-4 text-center">
				<p class="text-2xl font-bold text-gray-900">{formatCost(latest.bac || 0)}</p>
				<p class="text-xs text-gray-500 uppercase mt-1">BAC</p>
			</div>
			<div class="bg-white rounded-lg border border-gray-200 p-4 text-center">
				<p class="text-2xl font-bold text-gray-900">{formatCost(latest.eac || 0)}</p>
				<p class="text-xs text-gray-500 uppercase mt-1">EAC</p>
		...
		{#if latest.pv_cumulative || latest.ev_cumulative || latest.ac_cumulative}
			...
			<div class="bg-white rounded-lg border border-gray-200 p-6 mb-6">
				<p class="text-sm font-semibold text-gray-700 mb-3">Earned Value S-Curve</p>
			...
			<div class="overflow-x-auto">
				<table class="w-full text-sm">
					<thead class="bg-gray-50">
						<tr>
							<th class="px-4 py-3 text-left font-medium text-gray-500 uppercase text-xs">Project</th>
							<th class="px-4 py-3 text-right font-medium text-gray-500 uppercase text-xs">BAC</th>
							<th class="px-4 py-3 text-center font-medium text-gray-500 uppercase text-xs">SPI</th>
							<th class="px-4 py-3 text-center font-medium text-gray-500 uppercase text-xs">CPI</th>
							<th class="px-4 py-3 text-center font-medium text-gray-500 uppercase text-xs">Schedule</th>
							<th class="px-4 py-3 text-center font-medium text-gray-500 uppercase text-xs">Cost</th>
							<th class="px-4 py-3 text-right font-medium text-gray-500 uppercase text-xs">EAC</th>
						</tr>
					</thead>
					<tbody class="divide-y divide-gray-200">
						{#each analyses as a}
							<tr class="hover:bg-gray-50 cursor-pointer" onclick={() => window.location.href = `/evm/${a.analysis_id}`}>
								<td class="px-4 py-3 font-medium text-gray-900">{a.project_name || a.project_id}</td>
								<td class="px-4 py-3 text-right font-mono text-gray-500">{formatCost(a.bac || 0)}</td>
								...
								<td class="px-4 py-3 text-right font-mono text-gray-500">{formatCost(a.eac || 0)}</td>
							</tr>
						{/each}
					</tbody>
				</table>
