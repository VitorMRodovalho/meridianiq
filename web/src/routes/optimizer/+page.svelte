...
<svelte:head>
	<title>{$t('page.optimizer')} - MeridianIQ</title>
</svelte:head>
...
<main class="max-w-6xl mx-auto px-4 py-8">
	<div class="mb-8">
		...
		<p class="text-xs text-gray-400 mt-1">{$t('optimizer.subtitle_citation')}</p>
	</div>
	...
	<div class="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 p-6 mb-6">
		<div class="flex items-end gap-4 flex-wrap">
			...
			<div class="w-32">
				<label for="gens" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{$t('optimizer.generations_label')}</label>
			...
			<div class="w-32">
				<label for="pop" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{$t('optimizer.population_label')}</label>
			...
			<button onclick={optimize} disabled={!selectedProject || loading}
				...
				{loading ? $t('optimizer.optimizing') : $t('optimizer.optimize_button')}
			</button>
			{#if selectedProject}
	...
	{#if result}
		<div class="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
			<div class="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 p-3 text-center">
				...
				<p class="text-xs text-gray-500 dark:text-gray-400 uppercase">{$t('optimizer.kpi_original')}</p>
			</div>
			<div class="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 p-3 text-center">
				<p class="text-lg font-bold text-green-600">{result.optimized_makespan}d</p>
				<p class="text-xs text-gray-500 dark:text-gray-400 uppercase">{$t('optimizer.kpi_optimized')}</p>
			</div>
			<div class="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 p-3 text-center">
				<p class="text-lg font-bold text-blue-600">{result.improvement_pct.toFixed(1)}%</p>
				<p class="text-xs text-gray-500 dark:text-gray-400 uppercase">{$t('optimizer.kpi_improvement')}</p>
			</div>
			<div class="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 p-3 text-center">
				<p class="text-lg font-bold text-gray-700 dark:text-gray-300">{result.generations}</p>
				<p class="text-xs text-gray-500 dark:text-gray-400 uppercase">{$t('optimizer.kpi_generations')}</p>
			</div>
			<div class="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 p-3 text-center">
				<p class="text-sm font-bold text-gray-700 dark:text-gray-300 capitalize">{result.best_priority_rule}</p>
				<p class="text-xs text-gray-500 dark:text-gray-400 uppercase">{$t('optimizer.kpi_best_rule')}</p>
			</div>
		</div>
		...
		{#if result.convergence.length > 1}
			<div class="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 p-6 mb-6">
				<p class="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">{$t('optimizer.convergence_heading')}</p>
				<svg viewBox="0 0 {W} {H}" class="w-full">
					...
					<text x={W / 2} y={H - 5} text-anchor="middle" class="text-[9px] fill-gray-400">{$t('optimizer.generation_axis')}</text>
				</svg>
				<div class="flex items-center gap-4 mt-2 text-xs text-gray-500 dark:text-gray-400">
					<div class="flex items-center gap-1"><span class="w-4 h-0.5 bg-green-500 rounded"></span> {$t('optimizer.best_fitness_legend')}</div>
				</div>
			</div>
		{/if}
		...
		{#if result.shifted_activities.length > 0}
			<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
				<BarChart data={shiftItems} title={$t('optimizer.shifts_chart_title')} />
				<div class="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
					<h2 class="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">{$t('optimizer.shifted_heading')} ({result.shifted_activities.length})</h2>
