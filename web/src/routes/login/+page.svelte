<script lang="ts">
	import { signInWithGoogle, signInWithAzure, signInWithLinkedIn } from '$lib/auth';
	...
	async function handleGoogle() {
		...
		try {
			...
			error = e instanceof Error ? e.message : $t('login.sign_in_failed');
			loading = false;
		}
	}
...
<svelte:head>
	<title>{$t('login.page_title')} — MeridianIQ</title>
</svelte:head>
...
<div class="min-h-screen bg-gray-50 dark:bg-gray-800 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
	<div class="sm:mx-auto sm:w-full sm:max-w-md">
		...
		<p class="mt-2 text-center text-sm text-gray-500 dark:text-gray-400">{$t('sidebar.platform_subtitle')}</p>
	</div>

	<div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
		<div class="bg-white dark:bg-gray-900 py-8 px-4 shadow rounded-lg sm:px-10 space-y-4">
			<h2 class="text-xl font-semibold text-gray-800 text-center">{$t('login.sign_in_title')}</h2>
