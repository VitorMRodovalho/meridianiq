...
<div class="p-8 max-w-5xl mx-auto">
	...
	{:else if org}
		<div class="mb-6">
			...
			<p class="text-sm text-gray-500 mt-1">{org.org_type} &middot; {members.length} member{members.length !== 1 ? 's' : ''}</p>
		</div>
							...
							<tr class="hover:bg-gray-50">
								<td class="px-4 py-3">
									<div class="flex items-center gap-3">
										...
										{/if}
										<div>
											<p class="text-sm font-medium text-gray-900">{m.user_profiles?.full_name || 'User'}</p>
											<p class="text-xs text-gray-500">{m.user_profiles?.email || ''}</p>
										</div>
									</div>
								</td>
								<td class="px-4 py-3">
		...
		{:else if activeTab === 'invite'}
			<div class="bg-white border border-gray-200 rounded-lg p-6">
				...
				{#if inviteError}
					<div class="p-3 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm mb-4">{inviteError}</div>
				{/if}
				{#if inviteSuccess}
					<div class="p-3 bg-green-50 border border-green-200 rounded-lg text-green-700 text-sm mb-4">{inviteSuccess}</div>
				{/if}
				<div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
					...
					<label class="block">
						...
						<select bind:value={inviteRole} class="mt-1 block w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
		...
		{:else if activeTab === 'audit'}
			<div class="bg-white border border-gray-200 rounded-lg overflow-hidden">
				{#if auditEntries.length === 0}
					<div class="p-8 text-center text-gray-500 text-sm">No audit entries yet.</div>
				{:else}
					<table class="min-w-full divide-y divide-gray-200">
						<thead class="bg-gray-50">
							<tr>
						...
						<tbody class="divide-y divide-gray-200">
							{#each auditEntries as entry}
								<tr class="hover:bg-gray-50">
									<td class="px-4 py-3 text-sm text-gray-500 whitespace-nowrap">{formatDate(entry.created_at)}</td>
									<td class="px-4 py-3 text-sm text-gray-900">{entry.user_profiles?.full_name || entry.user_profiles?.email || 'System'}</td>
									<td class="px-4 py-3 text-sm text-gray-700">{actionLabel(entry.action)}</td>
