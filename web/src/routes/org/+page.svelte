...
<div class="p-8 max-w-4xl mx-auto">
	<div class="flex items-center justify-between mb-8">
		<div>
			...
			<p class="text-sm text-gray-500 mt-1">Manage your teams and project sharing</p>
		</div>
		<button
	...
	{#if error}
		<div class="p-4 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm mb-6">{error}</div>
	{/if}
	...
	{:else if orgs.length === 0}
		<div class="bg-white border border-gray-200 rounded-lg p-8 text-center">
			...
			<p class="text-sm text-gray-500 mb-4">Create an organization to start collaborating with your team.</p>
		</div>
	{:else}
		<div class="space-y-3">
			{#each orgs as org}
				<a
					href="/org/{org.id}"
					class="block bg-white border border-gray-200 rounded-lg p-5 hover:border-blue-300 hover:shadow-md transition-all"
				>
					<div class="flex items-center justify-between">
						<div>
							<h3 class="font-semibold text-gray-900">{org.name}</h3>
							<p class="text-sm text-gray-500 mt-0.5">{typeLabel(org.org_type)}</p>
						</div>
