<script lang="ts">
	...
	interface NavSection { titleKey: string; items: NavItem[]; auth?: boolean; }
	...
	const navSections: NavSection[] = [
		{
			...
			items: [
				...
				{ href: '/upload', labelKey: 'nav.upload', icon: 'M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12' },
				{ href: '/projects', labelKey: 'nav.projects', icon: 'M9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2' },
				{ href: '/demo', labelKey: 'nav.demo', icon: 'M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z' },
			],
		},
		{
			titleKey: 'nav.section.schedule', auth: true,
			items: [
				{ href: '/schedule', labelKey: 'nav.gantt', icon: 'M4 6h16M4 10h16M4 14h16M4 18h16' },
				{ href: '/compare', labelKey: 'nav.compare', icon: 'M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4' },
		...
		{
			titleKey: 'nav.section.forensic', auth: true,
			items: [
				{ href: '/forensic', labelKey: 'nav.forensic_cpa', icon: 'M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z' },
				{ href: '/tia', labelKey: 'nav.tia', icon: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z' },
				{ href: '/delay-attribution', labelKey: 'nav.delay_attribution', icon: 'M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z' },
				{ href: '/root-cause', labelKey: 'nav.root_cause', icon: 'M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM10 7v3m0 0v3m0-3h3m-3 0H7' },
				{ href: '/narrative', labelKey: 'nav.narrative', icon: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z' },
			],
		},
		{
			titleKey: 'nav.section.risk', auth: true,
			items: [
		...
		},
		{
			...
			items: [
				...
				{ href: '/builder', labelKey: 'nav.builder', icon: 'M12 6V4m0 2a2 2 0 100 4m0-4a2 2 0 110 4m-6 8a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4m6 6v10m6-2a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4' },
				{ href: '/visualization', labelKey: 'nav.visualization', icon: 'M4 5a1 1 0 011-1h14a1 1 0 011 1v2a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM4 13a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H5a1 1 0 01-1-1v-6zM16 13a1 1 0 011-1h2a1 1 0 011 1v6a1 1 0 01-1 1h-2a1 1 0 01-1-1v-6z' },
			],
		},
		{
			titleKey: 'nav.section.enterprise', auth: true,
			items: [
				{ href: '/programs', labelKey: 'nav.programs', icon: 'M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10' },
				{ href: '/reports', labelKey: 'nav.reports', icon: 'M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z' },
				{ href: '/ips', labelKey: 'nav.ips', icon: 'M9 17V7m0 10a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2h2a2 2 0 012 2m0 10a2 2 0 002 2h2a2 2 0 002-2M9 7a2 2 0 012-2h2a2 2 0 012 2m0 10V7m0 10a2 2 0 002 2h2a2 2 0 002-2V7a2 2 0 00-2-2h-2a2 2 0 00-2 2' },
				{ href: '/recovery', labelKey: 'nav.recovery', icon: 'M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15' },
				{ href: '/milestones', labelKey: 'nav.milestones', icon: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z' },
				{ href: '/org', labelKey: 'nav.org', icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z' },
			],
		},
		{
			titleKey: '',
	...
	const resolvedNavSections = $derived(
		navSections.map(section => ({
			...section,
			title: section.titleKey ? $t(section.titleKey) : '',
			items: section.items.map(item => ({ ...item, label: $t(item.labelKey) })),
		}))
	);

	const filteredNavSections = $derived.by(() => {
		if (!sidebarSearch.trim()) return resolvedNavSections;
		const q = sidebarSearch.toLowerCase();
		return resolvedNavSections.map(section => ({
			...section,
			items: section.items.filter(item => item.label.toLowerCase().includes(q)),
		})).filter(section => section.items.length > 0 || !section.titleKey);
	});
</script>
	...
	">
		<div class="px-6 py-5 border-b border-gray-700 flex items-center justify-between">
			<a href="/" class="block" onclick={closeSidebar}>
				...
				<p class="text-xs text-gray-400 mt-0.5">{$t('sidebar.platform_subtitle')}</p>
			</a>
			<button
				onclick={closeSidebar}
			...
			{#each filteredNavSections as section, si}
				...
				{#if section.titleKey}
					...
					>
						...
						<svg
							class="w-3 h-3 text-gray-500 transition-transform {collapsed[section.titleKey] ? '-rotate-90' : ''}"
				...
				<div class="flex items-center gap-3 mb-3">
					...
					<div class="min-w-0 flex-1">
						<p class="text-xs font-medium text-gray-200 truncate">
							{$user.user_metadata?.full_name ?? $user.email ?? $t('sidebar.user_fallback')}
						</p>
		...
		<div class="px-6 py-4 border-t border-gray-700">
			<div class="flex items-center gap-2 mb-3">
				...
				<button
					...
					aria-label={$t('sidebar.dark_mode_aria')}
					title={$isDark ? $t('sidebar.toggle_light') : $t('sidebar.toggle_dark')}
				>
					{#if $isDark}
			...
			<a href="/docs" onclick={closeSidebar} class="text-xs text-gray-400 hover:text-white transition-colors">{$t('sidebar.docs')}</a>
			...
			<p class="text-xs text-gray-500 mt-0.5">v{__APP_VERSION__}</p>
		</div>
	</aside>
		...
		<svelte:boundary>
			...
			{#snippet failed(error, reset)}
				<div class="max-w-2xl mx-auto px-4 py-16 text-center">
					...
					<h2 class="text-xl font-bold text-gray-900 dark:text-gray-100 mb-2">{$t('error.title')}</h2>
					<p class="text-sm text-gray-600 dark:text-gray-400 mb-4">{(error instanceof Error ? error.message : null) ?? $t('error.subtitle')}</p>
		...
		>
			...
			>
				<div class="flex items-center justify-between mb-4">
					<h2 class="text-lg font-bold text-gray-900 dark:text-gray-100">{$t('shortcuts.title')}</h2>
					<button onclick={() => showShortcuts = false} class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300" aria-label={$t('shortcuts.close')}>
						<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
					</button>
				</div>
				<div class="space-y-3 text-sm">
					<p class="text-xs text-gray-500 uppercase font-semibold">{$t('shortcuts.global')}</p>
					<div class="flex justify-between"><span class="text-gray-600 dark:text-gray-400">{$t('shortcuts.toggle_dark')}</span><kbd class="px-2 py-0.5 bg-gray-100 dark:bg-gray-800 rounded text-xs font-mono">Ctrl+D</kbd></div>
					<div class="flex justify-between"><span class="text-gray-600 dark:text-gray-400">{$t('shortcuts.search_sidebar')}</span><kbd class="px-2 py-0.5 bg-gray-100 dark:bg-gray-800 rounded text-xs font-mono">/</kbd></div>
					<div class="flex justify-between"><span class="text-gray-600 dark:text-gray-400">{$t('shortcuts.show_shortcuts')}</span><kbd class="px-2 py-0.5 bg-gray-100 dark:bg-gray-800 rounded text-xs font-mono">?</kbd></div>
					<div class="flex justify-between"><span class="text-gray-600 dark:text-gray-400">{$t('shortcuts.close_modal')}</span><kbd class="px-2 py-0.5 bg-gray-100 dark:bg-gray-800 rounded text-xs font-mono">Esc</kbd></div>
					<div class="border-t border-gray-100 dark:border-gray-800 pt-3 mt-3">
						<p class="text-xs text-gray-500 uppercase font-semibold">{$t('shortcuts.schedule_viewer')}</p>
					</div>
					<div class="flex justify-between"><span class="text-gray-600 dark:text-gray-400">{$t('shortcuts.zoom_in')}</span><kbd class="px-2 py-0.5 bg-gray-100 dark:bg-gray-800 rounded text-xs font-mono">+</kbd></div>
					<div class="flex justify-between"><span class="text-gray-600 dark:text-gray-400">{$t('shortcuts.zoom_out')}</span><kbd class="px-2 py-0.5 bg-gray-100 dark:bg-gray-800 rounded text-xs font-mono">-</kbd></div>
					<div class="flex justify-between"><span class="text-gray-600 dark:text-gray-400">{$t('shortcuts.expand_all')}</span><kbd class="px-2 py-0.5 bg-gray-100 dark:bg-gray-800 rounded text-xs font-mono">E</kbd></div>
					<div class="flex justify-between"><span class="text-gray-600 dark:text-gray-400">{$t('shortcuts.collapse_all')}</span><kbd class="px-2 py-0.5 bg-gray-100 dark:bg-gray-800 rounded text-xs font-mono">C</kbd></div>
				</div>
			</div>
		</div>
	{/if}
