<script lang="ts">
	...
	onMount(async () => {
		try {
			...
			error = e instanceof Error ? e.message : $t('programs.load_failed');
		} finally {
			loading = false;
		}
	});
...
<svelte:head>
	<title>{$t('nav.programs')} | MeridianIQ</title>
</svelte:head>

<main class="max-w-7xl mx-auto px-4 py-8">
	<h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-1">{$t('nav.programs')}</h1>
	<p class="text-sm text-gray-500 dark:text-gray-400 mb-6">{$t('programs.subtitle')}</p>
	...
	{:else if programs.length === 0}
		<div class="text-center py-12 text-gray-400 dark:text-gray-600">
			<p class="text-lg mb-2">{$t('programs.empty_title')}</p>
			<p class="text-sm">{$t('programs.empty_hint')}</p>
			<a href="/upload" class="inline-block mt-4 px-4 py-2 bg-blue-600 text-white rounded-md text-sm font-medium hover:bg-blue-700">{$t('programs.upload_cta')}</a>
		</div>
	{:else}
